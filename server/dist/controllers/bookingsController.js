...
const createBooking = async (req, res) => {
    ...
    try {
        ...
        const missingFields = [];
        if (!experienceId)
            missingFields.push('experienceId');
        if (!name)
        ...
        const updated = await Experience_1.default.findOneAndUpdate({
            _id: experienceId,
            "dateSlots.date": date,
        ...
        // compute price
        const experience = await Experience_1.default.findById(experienceId).session(session);
        if (!experience) {
            await session.abortTransaction();
        ...
        let discount = 0;
        if (promoCode) {
            const promo = await Promo_1.default.findOne({ code: promoCode.toUpperCase() }).session(session);
            if (!promo) {
                await session.abortTransaction();
            ...
            if (promo.type === "PERCENT")
                ...
                discount = Math.min(promo.value, basePrice);
